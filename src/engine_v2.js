/* engine_v2 — layer-aware browser port of convert.py + compute.py + build.py.
   Extends the original 16-object engine to the v1.3 service layer (24 objects) and
   drives composition + counts from the instance's `layers` block when present, so
   future category additions surface automatically. Backward compatible with v1.2. */
(function(global){
// --- category -> short kind code (16 core + 8 service-layer, v1.3) ---
const TYPE2K={Institution:'inst',Product:'prod',Feature:'feat',Segment:'seg',Channel:'chan',RuleThreshold:'rule',Pricing:'price',Authority:'auth',Policy:'pol',Disclosure:'disc',DocumentRequirement:'doc',OfferPromotion:'offer',Consent:'cons',Source:'src',Customer:'cust',Account:'acct',
  VoiceProfile:'voice',ServiceStandard:'sstd',ServicePolicy:'spol',Workflow:'wf',IntentTaxonomy:'intent',KnowledgeArticle:'ka',EscalationPath:'esc',Entitlement:'ent'};
const KL={inst:'Institution',prod:'Product',feat:'Feature',seg:'Segment',chan:'Channel',rule:'Rule',price:'Pricing',auth:'Authority',pol:'Policy',disc:'Disclosure',doc:'Doc Req',offer:'Offer',cons:'Consent',src:'Source',cust:'Customer',acct:'Account',
  voice:'Voice Profile',sstd:'Service Standard',spol:'Service Policy',wf:'Workflow',intent:'Intent Taxonomy',ka:'Knowledge Article',esc:'Escalation Path',ent:'Entitlement'};
const FIELD={inst:'legal_name',prod:'features',feat:'type',seg:'definition_criteria',chan:'description',rule:'condition',price:'apy_summary',auth:'action',pol:'domain',disc:'type',doc:'applicant_type',offer:'incentive',cons:'name',src:'description',
  voice:'register',sstd:'name',spol:'name',wf:'trigger',intent:'name',ka:'name',esc:'name',ent:'grants'};
// display label per category (full -> shown)
const CATLABEL={RuleThreshold:'Rule / Threshold',DocumentRequirement:'Document req.',OfferPromotion:'Offer / Promo',Source:'Source (doc/system)',
  VoiceProfile:'Voice profile',ServiceStandard:'Service standard',ServicePolicy:'Service policy',Workflow:'Workflow',IntentTaxonomy:'Intent taxonomy',KnowledgeArticle:'Knowledge article',EscalationPath:'Escalation path',Entitlement:'Entitlement'};
// layer -> display group header (driven by the v1.3 `layers` block)
const LAYER_META=[
  ['catalog','Catalog &amp; offering','· who &amp; what','ent'],
  ['compliance','Compliance &amp; control','· machine-applied','rules'],
  ['service','Service &amp; runtime','· how agents serve','svc'],
  ['spine_empty_by_design','Member spine','· consent-gated, empty by design','spine'],
  ['provenance','Provenance','· sources','src']];
// fallback grouping when no `layers` block (v1.2 instances)
const FALLBACK_LAYERS={catalog:['Product','Feature','Channel','Segment','Customer','Account'],compliance:['RuleThreshold','Authority','Pricing','Policy','Disclosure','DocumentRequirement','Consent','OfferPromotion'],service:[],spine_empty_by_design:[],provenance:['Source','Institution']};
const COMP_ORDER=['Product','Feature','Channel','Segment','Customer','Account','RuleThreshold','Authority','Pricing','Policy','Disclosure','DocumentRequirement','Consent','OfferPromotion','Institution','Source','VoiceProfile','ServiceStandard','ServicePolicy','Workflow','IntentTaxonomy','KnowledgeArticle','EscalationPath','Entitlement'];
// readiness business groups — service categories fold into Service & Support
const GROUPS=[['Drive Growth',10,['Product','Pricing','OfferPromotion','Segment','Feature']],['Service & Support',12,['Channel','DocumentRequirement','Consent','Disclosure','VoiceProfile','ServiceStandard','ServicePolicy','Workflow','IntentTaxonomy','KnowledgeArticle','EscalationPath','Entitlement']],['Attain Operating Efficiency',13,['RuleThreshold','Authority','Source','Pricing']],['Manage Risk & Compliance',18,['Policy','Disclosure','Authority','RuleThreshold']]];
...
function convert(d){
  ...
  for(const t in d.nodes){const k=TYPE2K[t]||t.toLowerCase();
    for(const n of d.nodes[t]){const prov=n.source_provenance||{},conf=n.confidence_score,srcn=prov.source||'';
      ...
      nodes[n.id]={id:n.id,k:k,kl:KL[k]||t,name:n.name||titleFromId(n.id),nf:hasNF(n),conf:conf,
...
function compute(d){
  ...
  const counts={}; for(const t in nodes)counts[t]=(nodes[t]||[]).length;
  ...
  const ncat=Object.keys(nodes).length;
  ...
  return {counts,total,ncat,rel,oblig,regpacks,attested,gaps,groups,legend:[R,P,B]};
}
// build the composition "kinds" inner HTML, grouped by the instance's layers block
function compositionHTML(d,counts){
  const layers=d.layers||FALLBACK_LAYERS;
  let html='';
  for(const [key,header,tag,cls] of LAYER_META){
    const cats=layers[key]||[]; if(!cats.length)continue;
    const present=cats.filter(c=>(counts[c]!=null)); if(!present.length)continue;
    const maxc=Math.max.apply(null,present.map(c=>counts[c]||0).concat([1]))||1;
    let objs='';
    for(const c of present){const cnt=counts[c]||0,w=Math.round(100*cnt/maxc),
      empty=(c==='Customer'||c==='Account'),disp=cnt+(empty?'*':''),lbl=CATLABEL[c]||c;
      objs+='<div class="obj'+(empty?' empty':'')+'"><span class="on">'+E(lbl)+'</span><span class="bar"><i style="width:'+(empty?0:w)+'%"></i></span><span class="oc">'+disp+'</span></div>';}
    html+='<div class="kind '+cls+'"><div class="kh">'+header+' <span class="tag">'+tag+'</span></div><div class="objs">'+objs+'</div></div>';
  }
  return html;
}
function topbar(d){
...
function spliceKinds(s,inner){
  const open=s.indexOf('<div class="kinds">'); if(open<0)return s;
  const secEnd=s.indexOf('</section>',open); if(secEnd<0)return s;
  // find the </div> that closes .kinds: it's the last </div> before </section>
  const lastDiv=s.lastIndexOf('</div>',secEnd);
  const startInner=open+'<div class="kinds">'.length;
...
function buildPage(template,d){
  ...
  if(gi>=0)lines[gi]='var G='+JSON.stringify(g)+';window.__KG=G;'; let s=lines.join('\n');
  ...
  s=rep1(s,'<div class="val">445</div><div class="sub">across 16 object types</div>','<div class="val">'+o.total+'</div><div class="sub">across '+o.ncat+' object types</div>');
  ...
  // composition — rebuild the kinds block from layers + counts (handles new categories)
  s=rep1(s,'<span class="more" data-go="explorer">16 objects · 4 kinds</span>','<span class="more" data-go="explorer">'+o.ncat+' objects · '+(d.layers?Object.keys(d.layers).filter(k=>(d.layers[k]||[]).some(c=>o.counts[c]!=null)).length:4)+' kinds</span>');
  s=spliceKinds(s,compositionHTML(d,o.counts));
  ...
  // self-contained: don't depend on a network fetch firing onload — invoke init directly from window.__KG
  s=s.split('xhr.send();').join('try{xhr.onload&&xhr.onload();}catch(_e){}');
  s=s.split('xhr.send(null);').join('try{xhr.onload&&xhr.onload();}catch(_e){}');
  s=s.split('__x.send();').join('try{__x.onload&&__x.onload();}catch(_e){}');
  s=s.split('__x.send(null);').join('try{__x.onload&&__x.onload();}catch(_e){}');
...
global.KGEngine={convert,compute,topbar,provenance,buildPage,compositionHTML};
